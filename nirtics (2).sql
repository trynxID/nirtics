...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `amount`  AS SELECT count(`event`.`id_event`) AS `event`, (select count(`kategori`.`id_kategori`) from `kategori`) AS `kategori`, (select count(`metode_pembayaran`.`id_metode`) from `metode_pembayaran`) AS `metode`, (select count(`transaksi`.`id_transaksi`) from `transaksi`) AS `transaksi`, (select count(`user`.`id_user`) from `user` where `user`.`level` = 'user') AS `user` FROM `event` ;
